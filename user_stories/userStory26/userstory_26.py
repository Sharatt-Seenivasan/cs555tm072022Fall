  ...
  for index, row in individuals_dataframe.iterrows():
    if str(row['child']) != 'nan':
      ...
      if row['id'] not in children:
        print("ERROR: INDIVIDUAL: US26: " + row['id'] + " not in Family " +  row['child'] + " as a child.")
    if str(row['spouse']) != 'nan':
      ...
      if row['id'] != husband_id and row['id'] != wife_id:
        print("ERROR: INDIVIDUAL: US26: " + row['id'] + " not in Family " +  row['spouse'] + " as a spouse.")


  for index, row in family_dataframe.iterrows():
    if row['Husband ID'] not in individuals_dataframe['id'].values:
      print("ERROR: FAMILY: US26: Husband " + row['Husband ID'] + " not in table of individuals")
    if row['Wife ID'] not in individuals_dataframe['id'].values:
      print("ERROR: FAMILY: US26: Wife " + row['Wife ID'] + " not in table of individuals")  
